describe("Sistema Integrado de Gestão para Fundações de Amparo a Pesquisas", () => {
  ...
  it("Realiza login no sistema e submete uma proposta", () => {
    ...
    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-23-item-ods02-erradicar"] > .MuiListItemIcon-root > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check(); //Clica no campo de seleção de ODS 2 - Erradicar a Fome, Alcançar a Segurança Alimentar e Melhorar a Nutrição e Promover a Agricultura Sustentável
    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-25-item-agronegocios"] > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check(); //Clica no campo de seleção de Agronegócios
    ...
    cy.get('[data-cy="formularioPropostaInformacaoComplementar.pergunta-26"]')
      .click()
      .type("20/10/2025", { delay: 0 }); //Clica no campo de seleção de "A proposta está vinculada a um projeto de pesquisa ou inovação em andamento?"

    cy.get('[data-cy="formularioPropostaInformacaoComplementar.pergunta-27"]')
      .click()
      .type("Essa eh a ocupacao tecnica durante a realizacao do evento", {
        delay: 0,
      }); //Clica no campo de seleção de "Data de início do projeto"

    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-24-item-mei-faturamento"] > .MuiListItemIcon-root > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check(); //Clica no campo de seleção de MEI - Faturamento Anual de até R$ 81.000,00

    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-25-item-energias-renovav"] > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check(); //Clica no campo de seleção de Energias Renováveis e Eficiência Energética
    ...
    cy.get('[data-cy="formularioPropostaDescritiva.pergunta-19"]')
      .click()
      .type("Obras e Instalações Novas", { delay: 0 }); //Clica no campo de descrição e preenche com o texto "Descrição da Proposta de Teste"

    cy.get('[data-cy="formularioPropostaDescritiva.pergunta-2"]')
      .click()
      .type("Experiência do Coordenador", { delay: 0 }); //Clica no campo de descrição e preenche com o texto "Descrição da Proposta de Teste"

    cy.get('[data-cy="formularioPropostaDescritiva.pergunta-16"]')
      .click()
      .type("Riscos e Atividades", { delay: 0 }); //Clica no campo de descrição e preenche com o texto "Descrição do Problema"

    cy.get('[data-cy="formularioPropostaDescritiva.pergunta-3"]')
      .click()
      .type("Resumo da Proposta de Projeto", { delay: 0 }); //Clica no campo de descrição e preenche com o texto "Descrição do Problema"

    cy.get('[data-cy="formularioPropostaDescritiva.pergunta-5"]')
      .click()
      .type("Objetivo Geral", { delay: 0 }); //Clica no campo de descrição e preenche com o texto "Descrição do Problema"
    ...
    ).click(); //Clica na aba Diárias para acessar a seção de Diárias
    cy.wait(1000); //Aguarda 500ms para garantir que a página foi carregada completamente
    ...
    cy.get('[data-cy="rubricaDiariaUnsaved.numeroDiaria"]')
      .click()
      .type("15", { delay: 0 }); //Clica no campo de Quantidade

    cy.get('[data-cy="rubricaDiariaUnsaved.custoUnitario"]')
      .click()
      .type("10000", { delay: 0 }); //Clica no campo de Custo Unitário
    ...
    cy.get('[data-cy="add-button"]').click();
    cy.get('[data-cy="rubricaMaterialConsumoUnsaved.especificacao"]')
      .click()
      .type("Especificação de Teste", { delay: 0 }); //Clica no campo de Especificação e preenche com o texto "Especificação de Teste"
    cy.get('[data-cy="rubricaMaterialConsumoUnsaved.unidadeMedida"]').click();
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(1).click();
    cy.get('[data-cy="rubricaMaterialConsumoUnsaved.quantidade"]')
      .click()
      .type("10", { delay: 0 }); //Clica no campo de Quantidade e preenche com o valor "10"
    cy.get('[data-cy="rubricaMaterialConsumoUnsaved.custoUnitario"]')
      .click()
      .type("1000", { delay: 0 }); //Clica no campo de Custo Unitário e preenche com o valor "1000"
    ...
    cy.get('[data-cy="add-button"]').click();
    cy.get('[data-cy="rubricaMaterialPermanenteUnsaved.especificacao"]')
      .click()
      .type("Especificação de Teste", { delay: 0 }); //Clica no campo de Especificação e preenche com o texto "Especificação de Teste"
    cy.get('[data-cy="rubricaMaterialPermanenteUnsaved.tipoOrigem"]').click();
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(1).click();
    cy.get('[data-cy="rubricaMaterialPermanenteUnsaved.quantidade"]')
      .click()
      .type("10", { delay: 0 }); //Clica no campo de Quantidade e preenche com o valor "10"
    cy.get('[data-cy="rubricaMaterialPermanenteUnsaved.custoUnitario"]')
      .click()
      .type("1000", { delay: 0 }); //Clica no campo de Custo Unitário e preenche com o valor "1000"
    ...
    cy.get('[data-cy="rubricaPassagemUnsaved.estadoRegiaoOrigem"]')
      .click()
      .type("Centro-Oeste", { delay: 0 });
    cy.get('[data-cy="rubricaPassagemUnsaved.paisDestinoId"]').click();
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(1).click();
    cy.get('[data-cy="rubricaPassagemUnsaved.estadoRegiaoDestino"]')
      .click()
      .type("Norte", { delay: 0 });
    cy.get('[data-cy="rubricaPassagemUnsaved.tipo"]').click();
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(1).click();
    cy.get('[data-cy="rubricaPassagemUnsaved.quantidade"]')
      .click()
      .type("10", { delay: 0 }); //Clica no campo de Quantidade e preenche com o valor "10"
    cy.get('[data-cy="rubricaPassagemUnsaved.custoUnitario"]')
      .click()
      .type("1000", { delay: 0 }); //Clica no campo de Custo Unitário e preenche com o valor "1000"
    ...
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(0).click();
    cy.get('[data-cy="rubricaHospedagemAlimentacaoUnsaved.especificacao"]')
      .click()
      .type("Especificação de Teste", { delay: 0 });
    cy.get('[data-cy="rubricaHospedagemAlimentacaoUnsaved.quantidade"]')
      .click()
      .type("10", { delay: 0 }); //Clica no campo de Quantidade e preenche com o valor "10"
    cy.get('[data-cy="rubricaHospedagemAlimentacaoUnsaved.custoUnitario"]')
      .click()
      .type("1000", { delay: 0 }); //Clica no campo de Custo Unitário e preenche com o valor "1000"
    cy.get(
      '[data-cy="rubricaHospedagemAlimentacaoUnsaved.mesPrevisto"]'
    ).click(); //Clica no campo de Mês Previsto
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(3).click(); //Seleciona o terceiro Mês Previsto da lista de Meses Previsto
    ...
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(0).click();
    cy.get('[data-cy="rubricaServicoTerceiroUnsaved.valorTotal"]')
      .click()
      .type("1000", { delay: 0 }); //Clica no campo de Custo Unitário e preenche com o valor "1000"
    ...
    cy.get('[data-cy="add-button"]').click();
    cy.get('[data-cy="rubricaPessoalUnsaved.funcao"]')
      .click()
      .type("Função Exemplo", { delay: 0 });
    cy.get('[data-cy="rubricaPessoalUnsaved.formacaoProfissional"]')
      .click()
      .type("Formação Exemplo", { delay: 0 });
    cy.get('[data-cy="rubricaPessoalUnsaved.perfilDesejado"]')
      .click()
      .type("Perfil Desejado Exemplo", { delay: 0 });
    ...
    cy.get('.MuiAutocomplete-popper [role="option"]').eq(0).click();
    cy.get('[data-cy="rubricaPessoalUnsaved.custoHoraCustoMes"]')
      .click()
      .type("1000", { delay: 0 });
    cy.get('[data-cy="rubricaPessoalUnsaved.valorTotal"]')
      .click()
      .type("1000", { delay: 0 });
    ...
    cy.get('[data-cy="rubricaEncargoUnsaved.especificacao"]')
      .click()
      .type("Especificação de Teste", { delay: 0 });
    cy.get('[data-cy="rubricaEncargoUnsaved.valorTotal"]')
      .click()
      .type("1000", { delay: 0 });
    ...
    cy.get('[data-cy="rubricaBolsaUnsaved.justificativa"]')
      .click()
      .type("Justificativa de Teste", { delay: 0 }); //Clica no campo de Justificativa e preenche com o texto "Justificativa de Teste"
    ...
    cy.get('[data-cy="documentoPessoalPropostaAnexo-procure"]').selectFile(
      "cypress/fixtures/rgtest.pdf"
    );
    ...
    cy.get('[data-cy="documentoPropostaAnexo-procure"]').selectFile(
      "cypress/fixtures/documentoteste.csv"
    ); //Seleciona o arquivo "robbie.pdf" do diretório "cypress/fixtures"
    ...
    cy.get(
      '[data-cy="termos"] > .MuiListItemText-root > .MuiTypography-root'
    ).click();
    cy.wait(500);
    cy.get(
      '[data-cy="termo-de-aceite"] > .MuiListItemText-root > .MuiTypography-root'
    ).click(); //Clica na aba Termo de Aceite para acessar a seção de Termo de Aceite
    cy.wait(1000);
    ...
    cy.get(
      '[data-cy="caracterizacao"] > .MuiListItemText-root > .MuiTypography-root'
    ).click(); //Clica no botão "Informações Complementares" para acessar a seção de Informações Complementares da proposta
    cy.wait(500); //Aguarda 500ms para garantir que a página foi carregada completamente
    cy.get(
      '[data-cy="informacoes-complementares"] > .MuiListItemText-root > .MuiTypography-root'
    ).click(); //Clica na aba Informações Complementares para acessar a seção de Informações Complementares
    cy.wait(500); //Aguarda 500ms para garantir que a página foi carregada completamente
    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-25-item-agronegocios"] > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check(); //Clica no campo de seleção de Agronegócios
    cy.get(
      '[data-cy="formularioPropostaInformacaoComplementar.pergunta-25-item-saude-humana-e-o"] > .MuiButtonBase-root > .PrivateSwitchBase-input'
    ).check();
    cy.get('[data-cy="menu-salvar"]').click(); //Clica no botão "Salvar" para salvar as informações da proposta
